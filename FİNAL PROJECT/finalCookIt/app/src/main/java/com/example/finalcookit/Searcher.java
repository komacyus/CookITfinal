        ...
        for(int i = 0; i < Program.notCookableWithCurrents.size() ; i ++){
            ...
            for(int k = 0 ; k < Program.notCookableWithCurrents.get(i).getIngsInRecipe().size() ; k++){
                ...
                if(currIngredientsContainsThisIngredient(nameOfIngInRecipe)){
                    ...
                    if(amountOfIngInRecipe > amountOfCurrentRecipeWithSameName)
                    {
                        notEnoughIngredients.add(Program.notCookableWithCurrents.get(i).getIngsInRecipe().get(k));
                    }
            ...
            if(totalNumberOfNeededElements==1) {
                ...
                if(missingIngredients.size()==1) {
                    ...
                    Program.notCookableWithCurrents.get(i).setURLofMissingIngToCookThis(URLofMissingIngredient);
                    Program.notCookableWithCurrents.get(i).setInputTypeOfMissingIngredient(inputTypeOfMissingIngredient);
                    Program.cookableWithExtras.add(Program.notCookableWithCurrents.get(i));
                }
                    ...
                    String inputTypeOfMissingIngredient = notEnoughIngredients.get(0).getIngredient().getInputTypeString();
                    Program.ingredientTypes[Program.findIndexOnIngTypeList(nameOfMissingIngredient)].scrape();
