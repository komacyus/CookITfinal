...
import com.example.finalcookit.Breakfast_RecyclerViewAdapter;
import com.example.finalcookit.Program;
import com.example.finalcookit.Recipe;
import com.example.finalcookit.RecipeDetailsActivity;
import com.example.finalcookit.RecipeModel;
import com.example.finalcookit.RecyclerViewInterface;

import java.util.ArrayList;

public class SuggestMeMoreActivity extends AppCompatActivity implements RecyclerViewInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_suggest_me_more);
        suggestMoreExplain = findViewById(R.id.suggestmoretext);
        ...
        @SuppressLint({"MissingInflatedId", "LocalSuppress"}) RecyclerView suggestionRecyclerView = findViewById(R.id.recyclerMore);
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                suggestMoreExplain.setText(Program.toStringCookableWithExtras());
            }
        });
    ...
    public void onItemClick(int position) {
        Intent intent = new Intent( SuggestMeMoreActivity.this, RecipeDetailsActivity.class);
        intent.putExtra("NAME", sugge.get(position).getName());
        intent.putExtra("DESCRIPTION", sugge.get(position).getDescription());
        intent.putExtra("IMAGE", sugge.get(position).getImage());
    ...
    public int recipeToInteger(Recipe recipe){
        ...
        else if(recipe.getName().equalsIgnoreCase("lentil soup")){
            return 2;
        }
